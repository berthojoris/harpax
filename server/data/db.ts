...
type DatabaseGlobal = typeof globalThis & {
  workforceDatabase?: Database.Database;
};

const globalForDatabase = globalThis as DatabaseGlobal;

function getDatabasePath(): string {
  const configuredPath = process.env.WORKFORCE_DATABASE_PATH;
  if (configuredPath) return configuredPath;
  return path.join(process.cwd(), "data", "workforce.db");
}
...
export function getDatabase(): Database.Database {
  if (globalForDatabase.workforceDatabase) return globalForDatabase.workforceDatabase;
  ...
  globalForDatabase.workforceDatabase = database;
  return database;
}
