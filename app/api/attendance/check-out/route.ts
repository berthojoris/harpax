...
import { recordCheckOut, ServiceError } from "@/server/services/workforce.service";
...
export async function POST(request: Request) {
  ...
  try {
    const record = recordCheckOut(user.id, parsed.data, request.headers.get("user-agent"));
    return NextResponse.json({
      success: true,
      message: "Berhasil Absen Keluar",
      recordedAt: formatTimeWithZone(record.checkOutAt),
      record: {
        id: record.id,
